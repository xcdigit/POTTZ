...
class _ProductMasterManagementTableContentState
  ...
  // 初始化左侧按钮列表
  _initButtonLeftList(List buttonItemList, WarehouseMasterModel state) {
  ...
  // 初始化右侧按钮列表
  _initButtonRightList(List buttonItemList, WarehouseMasterModel state) {
    // 按钮列表
    List<Widget> buttonList = [];
    // 循环按钮单个列表
    for (int i = 0; i < buttonItemList.length; i++) {
      ...
      builder: (context, state) {
        ...
        return Column(
          children: [
            ...
            WMSTableWidget<WarehouseMasterBloc, WarehouseMasterModel>(
...
class _DeleteDialogState extends State<DeleteDialog> {
  ...
  Widget build(BuildContext context) {
    return WMSDiaLogWidget(
      ...
      contentText:
          "${WMSLocalizations.i18n(context)!.delivery_note_14}:${deletedata['id']}${WMSLocalizations.i18n(context)!.display_instruction_delete}",
