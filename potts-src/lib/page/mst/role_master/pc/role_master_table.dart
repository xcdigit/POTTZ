...
class RoleMasterTable extends StatefulWidget {
  const RoleMasterTable({super.key});

  @override
  State<RoleMasterTable> createState() => _RoleMasterTableState();
}

class _RoleMasterTableState extends State<RoleMasterTable> {
  @override
...
//表格tab
class RoleMasterTableTab extends StatefulWidget {
  const RoleMasterTableTab({super.key});

  @override
  State<RoleMasterTableTab> createState() => _RoleMasterTableTabState();
}

class _RoleMasterTableTabState extends State<RoleMasterTableTab> {
  // 初始化Tab列表
  List<Widget> _initTabList(tabItemList, RoleMasterModel state) {
    // Tab列表
    List<Widget> tabList = [];
    // 循环Tab单个列表
...
//表格内容
class RoleMasterTableContent extends StatefulWidget {
  const RoleMasterTableContent({super.key});

  @override
  State<RoleMasterTableContent> createState() => _RoleMasterTableContentState();
}

class _RoleMasterTableContentState extends State<RoleMasterTableContent> {
  // 删除弹窗
  _deleteDialog(int id, BuildContext parentContext) {
    RoleMasterBloc bloc = context.read<RoleMasterBloc>();
    showDialog(
      context: context,
      builder: (context) {
        return BlocProvider<RoleMasterBloc>.value(
          value: bloc,
            ...
            builder: (context, state) {
              return WMSDiaLogWidget(
                ...
                onPressedRight: () {
                  ...
                  context
                      .read<RoleMasterBloc>()
                      .add(DeleteRoleEvent(parentContext, id));
                  // 关闭弹窗
                  Navigator.pop(context);
                },
              );
  ...
  // 初始化左侧按钮列表
  _initButtonLeftList(List buttonItemList, RoleMasterModel state) {
                          ...
                          child: DropdownButton<String>(
                            ...
                            onChanged: (String? newValue) {
                              setState(() {
                                ...
                                context.read<RoleMasterBloc>().add(SetSortEvent(
  ...
  // 初始化右侧按钮列表
  _initButtonRightList(List buttonItemList, RoleMasterModel state) {
    // 按钮列表
    List<Widget> buttonList = [];
    // 循环按钮单个列表
    for (int i = 0; i < buttonItemList.length; i++) {
          ...
          child: GestureDetector(
            onTap: () {
              ...
              if (buttonItemList[i]['index'] == Config.NUMBER_ZERO) {
                ...
                // 登录按钮跳转页面
                GoRouter.of(context)
                    .push('/roleMaster/details/0/1')
                    .then((value) {
                  // 判断返回值
                  if (value == 'refresh return') {
                    // 初始化事件
                    context.read<RoleMasterBloc>().add(InitEvent());
                  }
                });
              }
            },
      ...
      builder: (context, state) {
        ...
        return Column(
          children: [
            ...
            WMSTableWidget<RoleMasterBloc, RoleMasterModel>(
              showCheckboxColumn: false,
              columns: [
                {
                  //ロールID
                  'key': 'id',
                  'title': WMSLocalizations.i18n(context)!.role_overview_id,
                },
                {
                  //ロール名称
                  'key': 'name',
                  'title': WMSLocalizations.i18n(context)!.role_overview_name,
                },
                {
                  //ロール説明
                  'key': 'description',
                  'title':
                      WMSLocalizations.i18n(context)!.role_overview_explain,
                }
              ],
