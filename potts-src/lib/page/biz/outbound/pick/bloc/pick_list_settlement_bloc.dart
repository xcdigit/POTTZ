...
import 'pick_list_settlement_model.dart';

// 事件
abstract class PickListSettlementEvent {}

// 初始化事件
class InitEvent extends PickListSettlementEvent {
  // 初始化事件
  InitEvent();
}

class PickListSettlementBloc
    extends Bloc<PickListSettlementEvent, PickListSettlementModel> {
  // 刷新补丁
  PickListSettlementModel clone(PickListSettlementModel src) {
    return PickListSettlementModel.clone(src);
  }

  PickListSettlementBloc(PickListSettlementModel state) : super(state) {
    // 初始化事件
