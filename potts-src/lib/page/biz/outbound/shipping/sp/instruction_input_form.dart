  ...
  Widget build(BuildContext context) {
    ...
    List _tabItemList = [
      ...
      },
      {
        ...
        'title':
            WMSLocalizations.i18n(context)!.instruction_input_tab_before_text,
      },
    ];
    return BlocBuilder<InstructionInputBloc, InstructionInputModel>(
      builder: (context, state) {
...
class InstructionInputFormButton extends StatefulWidget {
  ...
  InstructionInputFormButton({super.key, required this.state});
  ...
  List<Widget> _initButtonList(buttonItemList, InstructionInputModel state) {
    ...
    for (int i = 0; i < buttonItemList.length; i++) {
      ...
      buttonList.add(
        Container(
          height: 40,
          margin: i == 0 ? EdgeInsets.all(0) : EdgeInsets.fromLTRB(16, 0, 0, 0),
          child: OutlinedButton(
            style: buttonItemList[i]['index'] == Config.NUMBER_ZERO
                ? ButtonStyle(
              ...
              if (buttonItemList[i]['index'] == Config.NUMBER_ZERO) {
                ...
                if (state.shipId == -1) {
                  if (state.currentIndex == 0) {
                    ...
                    WMSCommonBlocUtils.tipTextToast(
                        WMSLocalizations.i18n(context)!
                                .instruction_input_tab_before_text +
                            WMSLocalizations.i18n(context)!.input_text);
                  }
                } else {
                  //跳转table页面
