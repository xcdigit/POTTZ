...
class _InquiryScheduleTableContentState
  ...
  // 初始化右侧按钮列表
  _initButtonRightList(List buttonItemList, InquiryScheduleModel state) {
    // 按钮列表
    List<Widget> buttonList = [];
    // 循环按钮单个列表
    for (int i = 0; i < buttonItemList.length; i++) {
            ...
            onTap: () {
              ...
              if (buttonItemList[i]['index'] == Config.NUMBER_ZERO) {
                ...
                if (state.checkedRecords().length != 0) {
                  // 内容列表
                  Map<String, dynamic> contentMap = new HashMap();
                  // 表格选中列表
                  ...
                  for (int i = 0; i < wmsRecordModelList.length; i++) {
                    // 内容列表
                    contentMap = wmsRecordModelList[i].data;
                    data.add(contentMap['id']);
                  }
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => (WMSAccountTicketWidget(
                        formKbn: Config.NUMBER_THREE.toString(),
                        idList: data,
                      )),
                    ),
                  );
                  ...
                  // _showTipDialog(
                  //   WMSLocalizations.i18n(context)!
                  //       .inquiry_schedule_print_select_one,
                  // );
                }
              }
            },
  ...
  _showTipDialog(String text) {
    return showDialog<bool>(
      ...
      builder: (context) {
        return WMSDiaLogWidget(
          titleText: WMSLocalizations.i18n(context)!.inquiry_schedule_print_tip,
          contentText: text,
