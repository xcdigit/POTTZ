...
// ignore: must_be_immutable
class MenuMasterFormContent extends StatefulWidget {
  int flag; //按钮跳转判断
  MenuMasterFormContent({super.key, required this.flag});

  @override
  State<MenuMasterFormContent> createState() => _MenuMasterFormContentState();
}

class _MenuMasterFormContentState extends State<MenuMasterFormContent> {
  @override
  Widget build(BuildContext context) {
    //商品数据取出
    ...
    return BlocBuilder<MenuMasterBloc, MenuMasterModel>(
      builder: (context, state) {
        if (currentFlag) {
          //明细按钮输入框不可输入
          if (widget.flag == 0) {
            context.read<MenuMasterBloc>().add(ShowSelectValueEvent(data, "2"));
            //控制刷新
            StoreProvider.of<WMSState>(context)
                .dispatch(RefreshCurrentFlagAction(false));
          } else {
            //登录和修正按钮，输入框可以输入
            context.read<MenuMasterBloc>().add(ShowSelectValueEvent(data, "1"));
            //控制刷新
            StoreProvider.of<WMSState>(context)
                .dispatch(RefreshCurrentFlagAction(false));
          }
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: MenuMasterFormButton(state: state),
                      ),
                    ),
                  ],
                ),
...
class MenuMasterFormButton extends StatefulWidget {
  ...
  MenuMasterFormButton({super.key, required this.state});
            ...
            child: OutlinedButton(
              ...
              onPressed: () {
                ...
                if (buttonItemList[i]['index'] == Config.NUMBER_ZERO) {
                  //清除表单数据
                  context.read<MenuMasterBloc>().add(ClearFormEvent());
  ...
  Widget build(BuildContext context) {
    ...
    List _buttonItemList = [
      ...
      },
      {
        ...
        'title': context.read<MenuMasterBloc>().state.formInfo['id'] == '' ||
                context.read<MenuMasterBloc>().state.formInfo['id'] == null ||
                context.read<MenuMasterBloc>().state.stateFlg == '2'
