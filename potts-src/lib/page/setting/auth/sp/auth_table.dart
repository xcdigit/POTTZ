...
class AuthTable extends StatefulWidget {
  ...
  @override
  State<AuthTable> createState() => _AuthTableState();
}
...
class AuthTableTab extends StatefulWidget {
  const AuthTableTab({super.key});

  @override
  State<AuthTableTab> createState() => _AuthTableTabState();
}

class _AuthTableTabState extends State<AuthTableTab> {
  // 初始化Tab列表
  List<Widget> _initTabList(List tabItemList, AuthMasterModel state) {
    // Tab列表
    List<Widget> tabList = [];
    // 循环Tab单个列表
...
class AuthTableContent extends StatefulWidget {
  const AuthTableContent({super.key});

  @override
  State<AuthTableContent> createState() => _AuthTableContentState();
}

class _AuthTableContentState extends State<AuthTableContent> {
  // 删除弹窗
  _deleteDialog(int id, BuildContext parentContext) {
    AuthMasterBloc bloc = context.read<AuthMasterBloc>();
    showDialog(
      context: context,
      builder: (context) {
        return BlocProvider<AuthMasterBloc>.value(
          value: bloc,
          child: BlocBuilder<AuthMasterBloc, AuthMasterModel>(
            builder: (context, state) {
              return WMSDiaLogWidget(
                ...
                contentText: WMSLocalizations.i18n(context)!.auth_Form_1 +
                    '：' +
                    id.toString() +
                    ' ' +
                ...
                onPressedRight: () {
                  ...
                  context
                      .read<AuthMasterBloc>()
                      .add(DeleteAuthEvent(parentContext, id));
                  // 关闭弹窗
                  Navigator.pop(context);
                },
              );
...
// 初始化左侧按钮列表
  _initButtonLeftList(List buttonItemList, AuthMasterModel state) {
    List<DropdownMenuItem<String>> getListData() {
      ...
      DropdownMenuItem<String> dropdownMenuItem2 = new DropdownMenuItem<String>(
        child: Container(
          width: double.infinity, // 设置宽度为无限
          child: Text(
            WMSLocalizations.i18n(context)!.auth_Table_2,
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
        ),
                          ...
                          child: DropdownButton<String>(
                            ...
                            onChanged: (String? newValue) {
                              setState(() {
                                ...
                                context.read<AuthMasterBloc>().add(SetSortEvent(
...
// 初始化右侧按钮列表
  _initButtonRightList(List buttonItemList, AuthMasterModel state) {
    // 按钮列表
    List<Widget> buttonList = [];
    // 循环按钮单个列表
    for (int i = 0; i < buttonItemList.length; i++) {
              ...
              child: GestureDetector(
                onTap: () {
                  ...
                  if (buttonItemList[i]['index'] == Config.NUMBER_ZERO) {
                    ...
                    // 登录按钮跳转页面
                    GoRouter.of(context)
                        .push('/authMaster/details/0/1')
                        .then((value) {
                      // 判断返回值
                      if (value == 'refresh return') {
                        // 初始化事件
                        context.read<AuthMasterBloc>().add(InitEvent());
                      }
                    });
                  }
                },
      ...
      builder: (context, state) {
        ...
        return Column(
          children: [
            ...
            WMSTableWidget<AuthMasterBloc, AuthMasterModel>(
              ...
              columns: [
                ...
                },
                {
                  ...
                  'key': 'role_name',
                  'width': 0.5,
                  'title': WMSLocalizations.i18n(context)!.auth_Table_2,
                },
                {
                  //メニュー名称
                  'key': 'menu_name',
                  'width': 0.5,
                  'title': WMSLocalizations.i18n(context)!.auth_Table_3,
                },
                {
                  //権限
                  'key': 'auth',
                  'width': 0.5,
                  'title': WMSLocalizations.i18n(context)!.auth_Table_4,
                },
              ],
              operatePopupOptions: [
                {
