...
final UserReducer = combineReducers<Login?>([
  TypedReducer<Login?, UpdateUserAction>(_updateLoaded),
]);

/// 如果有 UpdateUserAction 发起一个请求时
/// 就会调用到 _updateLoaded
/// _updateLoaded 这里接受一个新的userInfo，并返回
Login? _updateLoaded(Login? user, action) {
...
class UserInfoMiddleware implements MiddlewareClass<WMSState> {
  @override
...
Stream<dynamic> userInfoEpic(
    Stream<dynamic> actions, EpicStore<WMSState> store) {
