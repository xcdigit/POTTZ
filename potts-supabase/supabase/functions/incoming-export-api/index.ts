...
serve(async (req) => {
  ...
  try {
    ...
    // 查询入荷予定
    let supabseReceiveData: Map<string, any> = await supabaseClient.from('dtb_receive').select('*').eq('receive_kbn', '5').eq('del_kbn', '2').eq('csv_kbn', '2').eq('company_id', companyId);
    // 判断入荷予定是否异常
    if (supabseReceiveData['error'] != null) {
      // 返回
      throw supabseReceiveData['error']['message'];
    }
    // 入荷予定
    let receiveData: Array<any> = supabseReceiveData['data'] != null ? supabseReceiveData['data'] : [];
    ...
    // 循环入荷予定
    for (let i: number = 0; i < receiveData.length; i ++) {
      // 当前入荷予定
      let currentReceive: Map<string, any> = receiveData[i];

      // 更新入荷予定
      let supabseReceiveUpdateData: Map<string, any> = await supabaseClient.from('dtb_receive').update({'csv_kbn': '1'}).eq('id', currentReceive['id']).select('*');
      // 判断入荷予定是否异常
      if (supabseReceiveUpdateData['error'] != null) {
        // 返回
        throw supabseReceiveUpdateData['error']['message'];
      }
      // 入荷予定
      let receiveUpdateData: Array<any> = supabseReceiveUpdateData['data'] != null ? supabseReceiveUpdateData['data'] : [];

      // 判断入荷予定数量
      if (receiveUpdateData.length == 0) {
        // 返回
        throw '412';
      }
      ...
      for (let j: number = 0; j < titleArray.length; j ++) {
        ...
        // 返回单项
        returnItem[currentTitle] = currentReceive[currentTitle];
      }
      // 返回数据
